...
const fetcher = (url: string) => fetch(url).then((r) => r.json()).then((data: GetGroupStatisticsResponse) => {
  const stats: Record<number, TeamStats> = data?.teams?.reduce((acc, t) => {
    acc[t.id] = {
      teamId: t.id,
      name: `${t.player1.lastName}, ${t.player2.lastName}`,
      results: {},
      wins: 0,
      losses: 0,
      setsWon: 0,
